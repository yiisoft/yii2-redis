...
test-sentinel:
	...
	PHP_VERSION=$(filter-out $@,$(MAKECMDGOALS)) docker compose -f tests/docker/docker-compose.yml exec yii2-redis-php sh -c "php -v && composer update && vendor/bin/phpunit --coverage-clover=coverage.clover"
